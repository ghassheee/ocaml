...
let rec pr_Ty outer ctx   = function
    | TyAll(x,tyBound,tyT)      ->  let ctx',x = fresh ctx x in 
                                    obox();pr"∀";pr x;pr"<:";pr_Ty false ctx tyBound;pr".";
                                    ps();pr_Ty outer ctx' tyT;cbox()
    | TySome(x,tyBound,tyT)     ->  let ctx,x = fresh ctx x in 
                                    obox();pr"∃";pr x;pr"<:";pr_Ty false ctx tyBound;pr".";
                                    ps();pr_Ty outer ctx tyT;cbox()
...
let rec pr_Tm outer ctx  = function 
    ...
    | TmTAbs(_,x,tyA,t)         ->  let ctx',tyX = fresh ctx x in obox();
                                    pr"λ";pr tyX;pr"<:";pr_ty ctx' tyA;pr".";psbr((not(small t))||outer);
