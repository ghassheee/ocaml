...
let rec typeof ctx   t      = 
    ...
    match t with
    | TmTAbs(fi,x,tyBound,t)    ->  p"T-TABS        "; 
                                    let ctx' = addbind ctx x (BindTyVar(tyBound)) in 
                                    TyAll(x,tyBound,typeof ctx' t) 
    | TmPack(fi,tyT,t,tySome)   ->  p"T-PACK        "; ( match simplifyty ctx tySome with
        | TySome(x,tyBound,tyT')    ->  let tyU = typeof ctx t in 
    ...
    | TmUnpack(fi,x,f,some,t)   ->  p"T-UNPACKPACK  "; let tySome = typeof ctx some in 
                                    ( match simplifyty ctx tySome with
        |   TySome(y,tyBound,tyT')  ->  let ctx' = addbind ctx x (BindTyVar(tyBound)) in
                                        let ctx''= addbind ctx' f(BindTmVar(tyT'))    in 
                                        tyShift(-2)(typeof ctx'' t) 
        |   _                       ->  error fi "existential type expected")
    | TmTApp(fi,t,tyT)          ->  p"T-TAPP        ";
                                    let tyT1 = typeof ctx t in (match simplifyty ctx tyT1 with 
        | TyAll(x,tyBound,tyT')     -> tySubstTop tyT tyT'
...
let prbindty ctx = function
    ...
    | BindTyVar(tyT)            -> pr"<:"; pr_ty ctx tyT  
