...
let rec tyWalk onVar c          = let f = onVar in function 
    ...
    | TyAll(x,tyBound,tyT)      -> TyAll(x, tyWalk f c tyBound, tyWalk f(c+1)tyT)
    | TySome(x,tyBound,tyT)     -> TySome(x, tyWalk f c tyBound, tyWalk f(c+1)tyT)
    | tyT                       -> tyT
...
let rec tmWalk onVar onTy c   = let (f,g) = (onVar,onTy) in function 
    ...
    | TmTAbs(fi,x,tyA,t)        -> TmTAbs(fi,x,g c tyA,tmWalk f g c t) 
